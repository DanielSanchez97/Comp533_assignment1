...
public class TwoClientCorrectConnectionTestInputGenerator extends ABufferingTestInputGenerator {
	...
	private int noneseSetupStage = 0;
...
//			checkStr(MAIN_THREAD, "SelectorFactorySet"),
	...
	private static final Pattern[] nonsenseStages = {
...
//			checkStr(SELECT_THREAD, "SelectCalled")
	...
	public TwoClientCorrectConnectionTestInputGenerator(boolean doNIO, boolean doRMI, boolean doGIPC) {
		this.doNIO = doNIO;
		this.doRMI = doRMI;
		this.doGIPC = doGIPC;
	}
...
//			}
	...
	protected boolean checkServer(String line) {
		...
		if (!isServerSetupComplete()) {
			if (!isServerGIPCSetupComplete() && checkServerGIPCSetup(line)) {
				...
				used = true;
			} else if (!isNonsenseSetupComplete() && checkNonsense(line)) {
				used = true;
			}
		} else if (!areServerAcceptsComplete()) {
			boolean didServer = false;
	...
	public boolean isNonsenseFSMComplete() {
		return isNonsenseSetupComplete();
	}
